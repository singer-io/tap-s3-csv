...
class S3MissingKeyProperties(S3CSVBaseTest):
    ...
    def get_properties() -> dict:
        ...
        with open(get_resources_path("tap-s3-csv/missing_key_properties_config.json"), encoding='utf-8') as file:
            data = json.load(file)
    ...
    def test_missing_key_properties(self):
        self.assertRaisesRegex(Exception, ".*missing required headers.*", runner.run_check_job_and_check_status, self)
