...
class TestJsonlSupport(unittest.TestCase):
    ...
    def mock_sync_jsonl_file(config, s3_path, table_spec, stream):
        return 1

    def mock_sync_csv_file(config, s3_path, table_spec, stream):
        return 1
    ...
    def test_check_key_properties_and_date_overrides_for_jsonl_file_key_properties_not_available(self):
        try:
            table_spec = {'search_prefix': '', 'search_pattern': 'test\\/.*\\.jsonl', 'table_name': 'jsonl_table',
                          'key_properties': ['idea'], 'date_overrides': ['created_at'], 'delimiter': ','}
            jsonl_sample_records = [{"name": "test4", "id": "4", "tt": "tetete", "ab": "bb", "mark11": [
                "221", "222"], "student11":{"no": 5, "col": 6}}]
            s3_path = "test.jsonl"
            s3.check_key_properties_and_date_overrides_for_jsonl_file(
    ...
    def test_check_key_properties_and_date_overrides_for_jsonl_file_date_overrides_not_available(self):
        try:
            table_spec = {'search_prefix': '', 'search_pattern': 'test\\/.*\\.jsonl', 'table_name': 'jsonl_table',
                          'key_properties': ['id'], 'date_overrides': ['created_at'], 'delimiter': ','}
            jsonl_sample_records = [{"name": "test4", "id": "4", "tt": "tetete", "ab": "bb", "mark11": [
                "221", "222"], "student11":{"no": 5, "col": 6}}]
            s3_path = "test.jsonl"
            s3.check_key_properties_and_date_overrides_for_jsonl_file(
    ...
    def test_check_key_properties_and_date_overrides_for_jsonl_file_no_exception(self):
        try:
            table_spec = {'search_prefix': '', 'search_pattern': 'test\\/.*\\.jsonl', 'table_name': 'jsonl_table',
                          'key_properties': ['id'], 'date_overrides': ['created_at'], 'delimiter': ','}
            jsonl_sample_records = [{"name": "test4", "id": "4", "tt": "tetete", "ab": "bb", "mark11": [
                "221", "222"], "created_at":"24-12-1996", "student11":{"no": 5, "col": 6}}]
            s3_path = "test.jsonl"
            s3.check_key_properties_and_date_overrides_for_jsonl_file(
                table_spec, jsonl_sample_records, s3_path)
        except Exception as err:
    ...
    @mock.patch("tap_s3_csv.s3.check_key_properties_and_date_overrides_for_jsonl_file")
    def test_get_record_for_jsonl_called_in_sample_file_for_jsonl_file(self, mock_json_file_handler_5_records_for_s3, mock_get_records_for_jsonl, mock_key_properties):
        ...
        s3.sample_file(config, table_spec, s3_path, sample_rate)
        self.assertEqual(mock_get_records_for_jsonl.call_count, 1)

    @mock.patch("tap_s3_csv.s3.get_file_handle", side_effect=mock_json_file_handler_5_records_for_s3)
    @mock.patch("tap_s3_csv.s3.get_records_for_csv", side_effect=mock_get_records_for_csv)
    @mock.patch("singer_encodings.csv.get_row_iterator")
    def test_get_record_for_csv_called_in_sample_file_for_csv_file(self, mock_json_file_handler_5_records_for_s3, mock_get_records_for_csv, mock_key_properties):
        ...
        s3.sample_file(config, table_spec, s3_path, sample_rate)
        self.assertEqual(mock_get_records_for_csv.call_count, 1)

    @mock.patch("tap_s3_csv.s3.get_file_handle", side_effect=mock_json_file_handler_5_records_for_s3)
    @mock.patch("tap_s3_csv.s3.get_records_for_csv", side_effect=mock_get_records_for_csv)
    @mock.patch("singer_encodings.csv.get_row_iterator")
    def test_get_record_for_csv_called_in_sample_file_for_txt_file(self, mock_json_file_handler_5_records_for_s3, mock_get_records_for_csv, mock_key_properties):
    ...
    def test_sync_jsonl_file_called_in_sync_table_file_for_jsonl_file(self, mock_sync_jsonl_file):
    ...
    def test_sync_csv_file_called_in_sync_table_file_for_csv_file(self, mock_sync_csv_file):
    ...
    @mock.patch("tap_s3_csv.sync.sync_csv_file", side_effect=mock_sync_csv_file)
    def test_sync_csv_file_called_in_sync_table_file_for_txt_file(self, mock_sync_csv_file):
